...
final class ImageProgressiveProvider: DataReceivingSideEffect {
    ...
    init?(_ options: GHKingfisherParsedOptionsInfo,
    ...
    func decode(_ data: Data,
        ...
        if option.isBlur, count < 6 {
            processingQueue.execute {
                ...
                let image = GHKingfisherWrapper<CFCrossPlatformImage>.image(
                    data: data,
...
private final class ImageProgressiveSerialQueue {
    ...
    private let queue: DispatchQueue = .init(label: "com.onevcat.GHKingfisher.ImageProgressive.SerialQueue")
