...
extension GHKingfisherWrapper where Base: CFCrossPlatformImage {
                      ...
                      backgroundColor: CFCrossPlatformColor? = nil) -> CFCrossPlatformImage
    {
        guard let _ = cgImage else {
            assertionFailure("[GHKingfisher] Blend mode image only works for CG-based image.")
            return base
        }
    ...
    {
        guard let _ = cgImage else {
            assertionFailure("[GHKingfisher] Compositing Operation image only works for CG-based image.")
            return base
        }
    ...
    {
        guard let _ = cgImage else {
            assertionFailure("[GHKingfisher] Round corner image only works for CG-based image.")
            return base
        }
        ...
        return draw(to: size) { _ in
            ...
            guard let context = UIGraphicsGetCurrentContext() else {
                assertionFailure("[GHKingfisher] Failed to create CG context for image.")
                return false
            }
    ...
    ///         For any non-CG-based image, `base` itself is returned.
     func resize(to size: CGSize) -> CFCrossPlatformImage {
        guard let _ = cgImage else {
            assertionFailure("[GHKingfisher] Resize only works for CG-based image.")
            return base
        }
    ...
    ///         For any non-CG-based image, `base` itself is returned.
     func crop(to size: CGSize, anchorOn anchor: CGPoint) -> CFCrossPlatformImage {
        guard let cgImage = cgImage else {
            assertionFailure("[GHKingfisher] Crop only works for CG-based image.")
            return base
        }
        
        let rect = self.size.cf.constrainedRect(for: size, anchor: anchor)
        guard let image = cgImage.cropping(to: rect.scaled(scale)) else {
            assertionFailure("[GHKingfisher] Cropping image failed.")
            return base
        }
        
        return GHKingfisherWrapper.image(cgImage: image, scale: scale, refImage: base)
    }
    ...
    ///         For any non-CG-based image, `base` itself is returned.
     func blurred(withRadius radius: CGFloat) -> CFCrossPlatformImage {
        ...
        guard let cgImage = cgImage else {
            assertionFailure("[GHKingfisher] Blur only works for CG-based image.")
            return base
        }
        ...
        // See https://github.com/onevcat/GHKingfisher/issues/611
        ...
        guard let context = beginContext(size: size, scale: scale, inverting: true) else {
            assertionFailure("[GHKingfisher] Failed to create CG context for blurring image.")
            return base
        }
        ...
        guard let blurredImage = result else {
            assertionFailure("[GHKingfisher] Can not make an blurred image within this context.")
            return base
        }
    ...
    ///         For any non-CG-based image, `base` itself is returned.
     func overlaying(with color: CFCrossPlatformColor, fraction: CGFloat) -> CFCrossPlatformImage {
        ...
        guard let _ = cgImage else {
            assertionFailure("[GHKingfisher] Overlaying only works for CG-based image.")
            return base
        }
    ...
    /// - Returns: The image with target scale. If the base image is already in the scale, `base` will be returned.
     func scaled(to scale: CGFloat) -> CFCrossPlatformImage {
        ...
        guard let cgImage = cgImage else {
            assertionFailure("[GHKingfisher] Scaling only works for CG-based image.")
            return base
        }
        return GHKingfisherWrapper.image(cgImage: cgImage, scale: scale, refImage: base)
    }
}
    ...
    ///         For any non-CG-based image or animated image, `base` itself is returned.
     func decoded(scale: CGFloat) -> CFCrossPlatformImage {
        ...
        guard let imageRef = cgImage else {
            assertionFailure("[GHKingfisher] Decoding only works for CG-based image.")
            return base
        }
...
extension GHKingfisherWrapper where Base: CFCrossPlatformImage {
    ...
    func beginContext(size: CGSize, scale: CGFloat, inverting: Bool = false) -> CGContext? {
        ...
        guard let rep = NSBitmapImageRep(
            ...
            bitsPerPixel: 0) else
        {
            assertionFailure("[GHKingfisher] Image representation cannot be created.")
            return nil
        }
        rep.size = size
        NSGraphicsContext.saveGraphicsState()
        guard let context = NSGraphicsContext(bitmapImageRep: rep) else {
            assertionFailure("[GHKingfisher] Image context cannot be created.")
            return nil
        }
    ...
    ) -> CFCrossPlatformImage
    {
        ...
        return GHKingfisherWrapper.image(cgImage: cgImage, scale: targetScale, refImage: ref)
    }
