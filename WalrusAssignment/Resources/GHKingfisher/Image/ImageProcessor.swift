...
/// An `ImageProcessor` would be used to convert some downloaded data to an image.
 protocol ImageProcessor {
    ...
    /// `GHKingfisherParsedOptionsInfo` as parameter instead.
    @available(*, deprecated,
    message: "Deprecated. Implement the method with same name but with `GHKingfisherParsedOptionsInfo` instead.")
    func process(item: ImageProcessItem, options: GHKingfisherOptionsInfo) -> CFCrossPlatformImage?
...
extension ImageProcessor {
     func process(item: ImageProcessItem, options: GHKingfisherOptionsInfo) -> CFCrossPlatformImage? {
        return process(item: item, options: GHKingfisherParsedOptionsInfo(options))
    }
}
...
typealias ProcessorImp = ((ImageProcessItem, GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage?)
 ...
 struct DefaultImageProcessor: ImageProcessor {
    ...
    /// - Note: See documentation of `ImageProcessor` protocol for more.
     func process(item: ImageProcessItem, options: GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage? {
        ...
        case .data(let data):
            return GHKingfisherWrapper.image(data: data, options: options.imageCreatingOptions)
        }
    }
}
 ...
 struct BlendImageProcessor: ImageProcessor {
    ...
    ///   - backgroundColor: Background color to apply for the output image. Default is `nil`.
     init(blendMode: CGBlendMode, alpha: CGFloat = 1.0, backgroundColor: CFCrossPlatformColor? = nil) {
        ...
        var identifier = "com.onevcat.GHKingfisher.BlendImageProcessor(\(blendMode.rawValue),\(alpha))"
...
/// Processor for adding an compositing operation to images. Only CG-based images are supported in macOS.
 struct CompositingImageProcessor: ImageProcessor {
                ...
                backgroundColor: CFCrossPlatformColor? = nil)
    {
        ...
        self.alpha = alpha
        self.backgroundColor = backgroundColor
        var identifier = "com.onevcat.GHKingfisher.CompositingImageProcessor(\(compositingOperation.rawValue),\(alpha))"
        if let color = backgroundColor {
            identifier.append("_\(color.hex)")
        }
        self.identifier = identifier
...
/// You could use `FormatIndicatedCacheSerializer.png` to force GHKingfisher to serialize the image to PNG format in this
/// case.
        ...
        backgroundColor: CFCrossPlatformColor? = nil)
    {
        ...
        self.identifier = {
            ...
            if let size = targetSize {
                identifier = "com.onevcat.GHKingfisher.RoundCornerImageProcessor" +
 ...
 struct ResizingImageProcessor: ImageProcessor {
    ...
    ///   a `CroppingImageProcessor`.
     init(referenceSize: CGSize, mode: ContentMode = .none) {
        ...
        if mode == .none {
            self.identifier = "com.onevcat.GHKingfisher.ResizingImageProcessor(\(referenceSize))"
        } else {
            self.identifier = "com.onevcat.GHKingfisher.ResizingImageProcessor(\(referenceSize), \(mode))"
        }
    }
 ...
 struct BlurImageProcessor: ImageProcessor {
    ...
    /// - parameter blurRadius: Blur radius for the simulated Gaussian blur.
     init(blurRadius: CGFloat) {
        ...
        self.identifier = "com.onevcat.GHKingfisher.BlurImageProcessor(\(blurRadius))"
    }
 ...
 struct OverlayImageProcessor: ImageProcessor {
    ...
    ///                       From 0.0 to 1.0. 0.0 means solid color, 1.0 means transparent overlay.
     init(overlay: CFCrossPlatformColor, fraction: CGFloat = 0.5) {
        ...
        self.identifier = "com.onevcat.GHKingfisher.OverlayImageProcessor(\(overlay.hex)_\(fraction))"
    }
 ...
 struct TintImageProcessor: ImageProcessor {
    ...
    /// - parameter tint: Tint color will be used to tint the input image.
     init(tint: CFCrossPlatformColor) {
        self.tint = tint
        self.identifier = "com.onevcat.GHKingfisher.TintImageProcessor(\(tint.hex))"
    }
 ...
 struct ColorControlsProcessor: ImageProcessor {
    ...
    ///   - inputEV: InputEV changing to image.
     init(brightness: CGFloat, contrast: CGFloat, saturation: CGFloat, inputEV: CGFloat) {
        ...
        self.identifier = "com.onevcat.GHKingfisher.ColorControlsProcessor(\(brightness)_\(contrast)_\(saturation)_\(inputEV))"
    }
...
/// watchOS is not supported.
 struct BlackWhiteProcessor: ImageProcessor {
    ...
    /// Identifier of the processor.
    /// - Note: See documentation of `ImageProcessor` protocol for more.
     let identifier = "com.onevcat.GHKingfisher.BlackWhiteProcessor"
 ...
 struct CroppingImageProcessor: ImageProcessor {
    ...
    ///   - while with a (1.0, 1.0) anchor (bottom-right), it will be `{80, 80, 20, 20}`
     init(size: CGSize, anchor: CGPoint = CGPoint(x: 0.5, y: 0.5)) {
        self.size = size
        self.anchor = anchor
        self.identifier = "com.onevcat.GHKingfisher.CroppingImageProcessor(\(size)_\(anchor))"
    }
...
/// Only CG-based images are supported. Animated images (like GIF) is not supported.
 struct DownsamplingImageProcessor: ImageProcessor {
    ...
    /// - Parameter size: The target size of the downsample operation.
     init(size: CGSize) {
        self.size = size
        self.identifier = "com.onevcat.GHKingfisher.DownsamplingImageProcessor(\(size))"
    }
    ...
    /// - Note: See documentation of `ImageProcessor` protocol for more.
     func process(item: ImageProcessItem, options: GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage? {
        ...
        case .image(let image):
            ...
            return GHKingfisherWrapper.downsampledImage(data: data, to: size, scale: options.scaleFactor)
        case .data(let data):
            return GHKingfisherWrapper.downsampledImage(data: data, to: size, scale: options.scaleFactor)
        }
    }
