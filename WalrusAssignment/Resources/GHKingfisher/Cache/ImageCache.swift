...
extension Notification.Name {
    ...
    /// `GHKingfisherDiskCacheCleanedHashKey` key in `userInfo` of the notification object you received.
    /// By checking the array, you could know the hash codes of files are removed.
     static let GHKingfisherDidCleanDiskCache =
        Notification.Name("com.onevcat.GHKingfisher.GHKingfisherDidCleanDiskCache")
}

/// Key for array of cleaned hashes in `userInfo` of `GHKingfisherDidCleanDiskCacheNotification`.
 let GHKingfisherDiskCacheCleanedHashKey = "com.onevcat.GHKingfisher.cleanedHash"
...
/// Represents the caching operation result.
 struct CacheStoreResult {
    ...
    /// you can get it from `.failure` case of this `diskCacheResult`.
     let diskCacheResult: Result<(), GHKingfisherError>
}
...
///
/// While a default image cache object will be used if you prefer the extension methods of GHKingfisher, you can create
...
/// the memory and disk storage config.
 class ImageCache {
    ...
    // MARK: Singleton
    /// The default `ImageCache` object. GHKingfisher will use this cache for its related methods if there is no
        ...
        diskStorage: DiskStorage.Backend<Data>)
    {
        ...
        let ioQueueName = "com.onevcat.GHKingfisher.ImageCache.ioQueue.\(UUID().uuidString)"
   ...
   convenience init(name: String) {
    ...
    {
        if name.isEmpty {
            fatalError("[GHKingfisher] You should specify a name for the cache. A cache with empty name is not permitted.")
        }
                    ...
                    completionHandler: ((CacheStoreResult) -> Void)? = nil)
    {
        ...
        ioQueue.async {
            ...
            if let data = serializer.data(with: image, original: original) {
                ...
                let diskError = GHKingfisherError.cacheError(
    ...
    ///               further use. By default, GHKingfisher uses a `DefaultCacheSerializer` to serialize the image to
                      ...
                      completionHandler: ((CacheStoreResult) -> Void)? = nil)
    {
        struct TempProcessor: ImageProcessor {
            ...
            func process(item: ImageProcessItem, options: GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage? {
                return nil
            }
        }
        
        let options = GHKingfisherParsedOptionsInfo([
        ...
        completionHandler: ((CacheStoreResult) -> Void)? = nil)
    {
        ...
        do {
            ...
            let diskError: GHKingfisherError
            if let error = error as? GHKingfisherError {
                diskError = error
            } else {
    ...
    func retrieveImage(forKey key: String,
                       ...
                       completionHandler: ((Result<ImageCacheResult, GHKingfisherError>) -> Void)?)
    {
    ...
    /// - Parameters:
    ///   - key: The key used for caching the image.
    ///   - options: The `GHKingfisherOptionsInfo` options setting used for retrieving the image.
    ...
    ///                        will be sent to this closure as result. Otherwise, a `GHKingfisherError` result
                     ...
                     completionHandler: ((Result<ImageCacheResult, GHKingfisherError>) -> Void)?)
    {
        retrieveImage(
            forKey: key,
            options: GHKingfisherParsedOptionsInfo(options),
            callbackQueue: callbackQueue,
            completionHandler: completionHandler)
    }
    ...
    func retrieveImageInMemoryCache(
        forKey key: String,
        options: GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage?
    {
    ...
    ///            has already expired, `nil` is returned.
     func retrieveImageInMemoryCache(
        forKey key: String,
        options: GHKingfisherOptionsInfo? = nil) -> CFCrossPlatformImage?
    {
        return retrieveImageInMemoryCache(forKey: key, options: GHKingfisherParsedOptionsInfo(options))
    }
    ...
    func retrieveImageInDiskCache(
        ...
        options: GHKingfisherParsedOptionsInfo,
        callbackQueue: CallbackQueue = .untouch,
        completionHandler: @escaping (Result<CFCrossPlatformImage?, GHKingfisherError>) -> Void)
    {
        let computedKey = key.computedKey(with: options.processor.identifier)
        ...
        loadingQueue.execute {
            do {
                ...
                if let error = error as? GHKingfisherError {
                    ...
                    assertionFailure("The internal thrown error should be a `GHKingfisherError`.")
                }
            }
        }
    }
                ...
                if !removed.isEmpty {
                    DispatchQueue.main.async {
                        ...
                        NotificationCenter.default.post(
                            name: .GHKingfisherDidCleanDiskCache,
                            object: self,
                            userInfo: [GHKingfisherDiskCacheCleanedHashKey: cleanedHashes])
                    }
                }
    ...
    @objc  func backgroundCleanExpiredDiskCache() {
        ...
        guard let sharedApplication = GHKingfisherWrapper<UIApplication>.shared else { return }
    ...
    /// - Parameter handler: Called with the size calculating finishes. This closure is invoked from the main queue.
     func calculateDiskStorageSize(completion handler: @escaping ((Result<UInt, GHKingfisherError>) -> Void)) {
        ioQueue.async {
            do {
...
extension UIApplication: GHKingfisherCompatible { }
extension GHKingfisherWrapper where Base: UIApplication {
