...
/// for storing to the disk storage.
 protocol CacheSerializer {
    ...
    func image(with data: Data, options: GHKingfisherParsedOptionsInfo) -> CFCrossPlatformImage?
    ...
    func image(with data: Data, options: GHKingfisherOptionsInfo?) -> CFCrossPlatformImage?
}

extension CacheSerializer {
     func image(with data: Data, options: GHKingfisherOptionsInfo?) -> CFCrossPlatformImage? {
        return image(with: data, options: GHKingfisherParsedOptionsInfo(options))
    }
}

/// Represents a basic and default `CacheSerializer` used in GHKingfisher disk cache system.
...
/// image other than these formats, a normalized `pngRepresentation` will be used.
 struct DefaultCacheSerializer: CacheSerializer {
    ...
    /// The default general cache serializer used across GHKingfisher's cache.
