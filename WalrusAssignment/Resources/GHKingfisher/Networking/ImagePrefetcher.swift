...
/// and caching before they display on screen.
   class ImagePrefetcher: CustomStringConvertible {
    ...
    private let optionsInfo: GHKingfisherParsedOptionsInfo
    ...
    private let manager: GHKingfisherManager

    private let pretchQueue = DispatchQueue(label: "com.onevcat.GHKingfisher.ImagePrefetcher.pretchQueue")
    private static let requestingQueue = DispatchQueue(label: "com.onevcat.GHKingfisher.ImagePrefetcher.requestingQueue")
    ...
    ///   - options: Options could control some behaviors. See `GHKingfisherOptionsInfo` for more.
    ...
    /// a customized `GHKingfisherOptionsInfo`. Both the progress and completion block will be invoked in
    ...
    init(sources: [Source], options: GHKingfisherOptionsInfo?) {
        var options = GHKingfisherParsedOptionsInfo(options)
        ...
        manager = GHKingfisherManager(downloader: downloader, cache: cache)
    }
    ...
    private func downloadAndCache(_ source: Source) {
        ...
        let downloadTaskCompletionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void) = { result in
