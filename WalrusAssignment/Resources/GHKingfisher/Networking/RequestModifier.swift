...
/// Represents and wraps a method for modifying request before an image download request starts.
 protocol ImageDownloadRequestModifier {
    ...
    /// `GHKingfisherOptionsInfoItem.requestModifier` and use it as the `options` parameter in related methods.
    ///
    ...
    ///            a `GHKingfisherError.requestError` with `.emptyRequest` as its reason will occur.
    ///
...
/// This type conforms to `ImageDownloadRequestModifier` and wraps an image modify block.
 struct AnyModifier: ImageDownloadRequestModifier {
    ...
    ///                     If `nil` returned, a `GHKingfisherError.requestError` with `.emptyRequest` as its
    ///                     reason will occur.
