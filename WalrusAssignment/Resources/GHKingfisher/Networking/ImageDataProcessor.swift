...
private let sharedProcessingQueue: CallbackQueue =
    .dispatch(DispatchQueue(label: "com.onevcat.GHKingfisher.ImageDownloader.Process"))
...
class ImageDataProcessor {
    ...
    let onImageProcessed = Delegate<(Result<CFCrossPlatformImage, GHKingfisherError>, SessionDataTask.TaskCallback), Void>()
    ...
    private func doProcess() {
        ...
        for callback in callbacks {
            ...
            let result: Result<CFCrossPlatformImage, GHKingfisherError>
            if let image = image {
