...
class SessionDelegate: NSObject {
    ...
    let onDownloadingFinished = Delegate<(URL, Result<URLResponse, GHKingfisherError>), Void>()
        ...
        callback: SessionDataTask.TaskCallback) -> DownloadTask
    {
        ...
        task.onCallbackCancelled.delegate(on: self) { [unowned task] (self, value) in
            ...
            let error = GHKingfisherError.requestError(reason: .taskCancelled(task: task, token: token))
...
extension SessionDelegate: URLSessionDataDelegate {
        ...
        completionHandler: @escaping (URLSession.ResponseDisposition) -> Void)
    {
        guard let httpResponse = response as? HTTPURLResponse else {
            let error = GHKingfisherError.responseError(reason: .invalidURLResponse(response: response))
        ...
        guard onValidStatusCode.call(httpStatusCode) == true else {
            let error = GHKingfisherError.responseError(reason: .invalidHTTPStatusCode(response: httpResponse))
            onCompleted(task: dataTask, result: .failure(error))
            completionHandler(.cancel)
            return
        }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let url = task.originalRequest?.url {
            let result: Result<URLResponse, GHKingfisherError>
            if let error = error {
                result = .failure(GHKingfisherError.responseError(reason: .URLSessionError(error: error)))
            } else if let response = task.response {
                result = .success(response)
            } else {
                result = .failure(GHKingfisherError.responseError(reason: .noURLResponse(task: sessionTask)))
            }
            onDownloadingFinished.call((url, result))
        }

        let result: Result<(Data, URLResponse?), GHKingfisherError>
        if let error = error {
            result = .failure(GHKingfisherError.responseError(reason: .URLSessionError(error: error)))
        } else {
            if let data = onDidDownloadData.call(sessionTask), let finalData = data {
                ...
                result = .failure(GHKingfisherError.responseError(reason: .dataModifyingFailed(task: sessionTask)))
            }
        }
    ...
    private func onCompleted(task: URLSessionTask, result: Result<(Data, URLResponse?), GHKingfisherError>) {
