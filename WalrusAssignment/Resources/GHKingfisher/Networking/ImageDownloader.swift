...
/// Represents a task of an image downloading process.
 struct DownloadTask {
    ...
    /// to a same `sessionTask`. This is an optimization in GHKingfisher to prevent multiple downloading task
    ...
    ///
    /// - Note:
    /// In GHKingfisher, there is an optimization to prevent starting another download task if the target URL is being
...
open class ImageDownloader {
    ...
    /// - Parameter name: The name for the downloader. It should not be empty.
     init(name: String) {
        if name.isEmpty {
            fatalError("[GHKingfisher] You should specify a name for the downloader. "
    ...
    /// Downloads an image with a URL and option. Invoked internally by GHKingfisher. Subclasses must invoke super.
    ///
    /// - Parameters:
    ///   - url: Target URL.
    ///   - options: The options could control download behavior. See `GHKingfisherOptionsInfo`.
    ...
    @discardableResult
     func downloadImage(
        ...
        completionHandler: ((Result<ImageLoadingResult, GHKingfisherError>) -> Void)? = nil) -> DownloadTask?
    {
        ...
        if let requestModifier = options.requestModifier {
            ...
            guard let r = requestModifier.modified(for: request) else {
                options.callbackQueue.execute {
                    completionHandler?(.failure(GHKingfisherError.requestError(reason: .emptyRequest)))
                }
                return nil
            }
            request = r
        ...
        guard let url = request.url, !url.absoluteString.isEmpty else {
            options.callbackQueue.execute {
                completionHandler?(.failure(GHKingfisherError.requestError(reason: .invalidURL(request: request))))
            }
            return nil
        }
        ...
        let onCompleted = completionHandler.map {
            block -> Delegate<Result<ImageLoadingResult, GHKingfisherError>, Void> in
            let delegate =  Delegate<Result<ImageLoadingResult, GHKingfisherError>, Void>()
