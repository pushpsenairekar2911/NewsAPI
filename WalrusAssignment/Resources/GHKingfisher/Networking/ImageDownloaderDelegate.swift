...
/// working stages and rules.
 protocol ImageDownloaderDelegate: AnyObject {
    ...
    /// your last chance to verify or modify the downloaded data before GHKingfisher tries to perform addition
    ...
    /// - Returns: The data from which GHKingfisher should use to create an image. You need to provide valid data
    ///            which content is one of the supported image file format. GHKingfisher will perform process on this
    ...
    ///   decrypting or verification). If `nil` returned, the processing is interrupted and a `GHKingfisherError` with
    ...
    /// If an invalid code is received, the downloader will raise an `GHKingfisherError` with
