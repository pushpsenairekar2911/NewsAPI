...
/// Represents the result of a GHKingfisher retrieving image task.
...
/// A struct that stores some related information of an `GHKingfisherError`. It provides some context information for
...
/// Main manager class of GHKingfisher. It connects GHKingfisher downloader and cache,
/// to provide a set of convenience methods to use GHKingfisher for tasks.
/// You can use this class to retrieve an image via a specified URL from web or cache.
@objc   class GHKingfisherManager: NSObject {

    /// Represents a shared manager used across GHKingfisher.
    /// Use this instance for getting or storing images with GHKingfisher.
     static let shared = GHKingfisherManager()

    // Mark:  Properties
    /// The `ImageCache` used by this manager. It is `ImageCache.default` by default.
    /// If a cache is specified in `GHKingfisherManager.defaultOptions`, the value in `defaultOptions` will be
    /// used instead.
     var cache: ImageCache
    
    /// The `ImageDownloader` used by this manager. It is `ImageDownloader.default` by default.
    /// If a downloader is specified in `GHKingfisherManager.defaultOptions`, the value in `defaultOptions` will be
    /// used instead.
     var downloader: ImageDownloader
    
    /// Default options used by the manager. This option will be used in
    /// GHKingfisher manager related methods, as well as all view extension methods.
    /// You can also passing other options for each image task by sending an `options` parameter
    /// to GHKingfisher's APIs. The per image options will overwrite the default ones,
    /// if the option exists in both.
     var defaultOptions = GHKingfisherOptionsInfo.empty
    
    // Use `defaultOptions` to overwrite the `downloader` and `cache`.
    private var currentDefaultOptions: GHKingfisherOptionsInfo {
    ...
    ///   - cache: The image cache which stores memory and disk images.
     init(downloader: ImageDownloader, cache: ImageCache) {
        ...
        let processQueueName = "com.onevcat.GHKingfisher.GHKingfisherManager.processQueue.\(UUID().uuidString)"
    ...
    @discardableResult
     func retrieveImage(
        ...
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> DownloadTask?
    {
        ...
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> DownloadTask?
    {
        ...
        var info = GHKingfisherParsedOptionsInfo(options)
    ...
    func retrieveImage(
        with source: Source,
        options: GHKingfisherParsedOptionsInfo,
        downloadTaskUpdated: DownloadTaskUpdatedBlock? = nil,
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> DownloadTask?
    {
        var context = RetrievingContext(options: options, originalSource: source)

        func handler(currentSource: Source, result: (Result<RetrieveImageResult, GHKingfisherError>)) -> Void {
            switch result {
            case .success:
            ...
            case .failure(let error):
                ...
                if let nextSource = context.popAlternativeSource() {
                    ...
                    if context.propagationErrors.isEmpty {
                        ...
                        context.appendError(error, to: currentSource)
                        let finalError = GHKingfisherError.imageSettingError(
        ...
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> DownloadTask?
    {
        ...
        if options.forceRefresh {
            ...
            if options.onlyFromCache {
                let error = GHKingfisherError.cacheError(reason: .imageNotExisting(key: source.cacheKey))
                completionHandler?(.failure(error))
                return nil
            }
    ...
    func provideImage(
        ...
        options: GHKingfisherParsedOptionsInfo,
        completionHandler: ((Result<ImageLoadingResult, GHKingfisherError>) -> Void)?)
    {
        ...
        provider.data { result in
                ...
                (options.processingQueue ?? self.processingQueue).execute {
                    ...
                    guard let image = processor.process(item: processingItem, options: options) else {
                        options.callbackQueue.execute {
                            let error = GHKingfisherError.processorError(
            ...
            case .failure(let error):
                options.callbackQueue.execute {
                    let error = GHKingfisherError.imageSettingError(
    ...
    private func cacheImage(
        ...
        options: GHKingfisherParsedOptionsInfo,
        context: RetrievingContext,
        result: Result<ImageLoadingResult, GHKingfisherError>,
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?
    )
    {
        switch result {
    ...
    func loadAndCacheImage(
        source: Source,
        context: RetrievingContext,
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> DownloadTask.WrappedTask?
    {
        let options = context.options
        func _cacheImage(_ result: Result<ImageLoadingResult, GHKingfisherError>) {
            cacheImage(
                source: source,
                options: options,
                context: context,
    ...
    ///    `options` will be considered when searching in the cache. If no processed image is found, GHKingfisher
    ///    will try to check whether an original version of that image is existing or not. If there is already an
    ///    original, GHKingfisher retrieves it from cache and processes it. Then, the processed image will be store
    ///    back to cache for later use.
    func retrieveImageFromCache(
        source: Source,
        context: RetrievingContext,
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)?) -> Bool
    {
        let options = context.options
                ...
                options.callbackQueue.execute {
                    result.match(
                        onSuccess: { cacheResult in
                            let value: Result<RetrieveImageResult, GHKingfisherError>
                            if let image = cacheResult.image {
                                ...
                                value = .failure(GHKingfisherError.cacheError(reason: .imageNotExisting(key: key)))
                            }
                            completionHandler(value)
                        },
                        onFailure: { _ in
                            completionHandler(.failure(GHKingfisherError.cacheError(reason: .imageNotExisting(key: key))))
                        }
                    )
                }
            }
            ...
            originalCache.retrieveImage(forKey: key, options: optionsWithoutProcessor) { result in
                ...
                result.match(
                    ...
                    onFailure: { _ in
                        ...
                        options.callbackQueue.execute {
                            completionHandler?(
                                .failure(GHKingfisherError.cacheError(reason: .imageNotExisting(key: key)))
                            )
                        }
                    }
                )
...
struct RetrievingContext {
    ...
    var options: GHKingfisherParsedOptionsInfo

    let originalSource: Source
    var propagationErrors: [PropagationError] = []

    init(options: GHKingfisherParsedOptionsInfo, originalSource: Source) {
    ...
    @discardableResult
    mutating func appendError(_ error: GHKingfisherError, to source: Source) -> [PropagationError] {
