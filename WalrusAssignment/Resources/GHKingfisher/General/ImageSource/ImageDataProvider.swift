...
/// Represents a data provider to provide image data to GHKingfisher when setting with
...
/// representation for the image.
 protocol ImageDataProvider {
    ...
    /// Provides the data which represents image. GHKingfisher uses the data you pass in the
    ...
    /// `ImageSettingErrorReason` will be finally thrown out to you as the `GHKingfisherError`
    /// from the framework.
    func data(handler: @escaping (Result<Data, Error>) -> Void)
}

/// Represents an image data provider for loading from a local file URL on disk.
/// Uses this type for adding a disk image to GHKingfisher. Compared to loading it
/// directly, you can get benefit of using GHKingfisher's extension methods, as well
/// as applying `ImageProcessor`s and storing the image to `ImageCache` of GHKingfisher.
...
/// Represents an image data provider for a raw data object.
 struct RawImageDataProvider: ImageDataProvider {
    ...
    /// The raw data object to provide to GHKingfisher image loader.
     let data: Data

    // MARK: Initializers

    /// Creates an image data provider by the given raw `data` value and a `cacheKey` be used in GHKingfisher cache.
    ///
    /// - Parameters:
