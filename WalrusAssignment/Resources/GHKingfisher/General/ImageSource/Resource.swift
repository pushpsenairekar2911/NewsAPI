...
/// GHKingfisher will use a `Resource` to download a resource from network and cache it with the cache key when
...
/// When passed to image view set methods, GHKingfisher will try to download the target
/// image from the `downloadURL`, and then store it with the `cacheKey` as the key in cache.
 struct ImageResource: Resource {
    ...
    ///   - cacheKey: The cache key. If `nil`, GHKingfisher will use the `absoluteString` of `downloadURL` as the key.
    ///               Default is `nil`.
...
/// URL conforms to `Resource` in GHKingfisher.
