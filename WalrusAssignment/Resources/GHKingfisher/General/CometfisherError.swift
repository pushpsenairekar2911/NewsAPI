...
/// Represents all the errors which can happen in GHKingfisher framework.
/// GHKingfisher related methods always throw a `GHKingfisherError` or invoke the callback with `GHKingfisherError`
/// as its error type. To handle errors from GHKingfisher, you switch over the error to get a reason catalog,
/// then switch over the reason to know error detail.
 enum GHKingfisherError: Error {
    ...
    /// Represents the error reason during GHKingfisher caching system.
    ///
    ...
    /// - cannotSetCacheFileAttribute: Cannot set file attributes to a cached file. Code 3010.
     enum CacheErrorReason {
        ...
        /// - key: The cache key used for the cache. When caching a file through `GHKingfisherManager` and GHKingfisher's
    ...
    /// - dataProviderError: An error happens during getting data from an `ImageDataProvider`. Code 5003.
     enum ImageSettingErrorReason {
        ...
        /// `.alternativeSources` are used and GHKingfisher tried to recovery from the original error, but still
...
extension GHKingfisherError: LocalizedError {
...
extension GHKingfisherError: CustomNSError {

    /// The error domain of `GHKingfisherError`. All errors from GHKingfisher is under this domain.
     static let domain = "com.onevcat.GHKingfisher.Error"
...
extension GHKingfisherError.RequestErrorReason {
    var errorDescription: String? {
        switch self {
        case .emptyRequest:
...
extension GHKingfisherError.ResponseErrorReason {
    var errorDescription: String? {
        switch self {
...
extension GHKingfisherError.CacheErrorReason {
    var errorDescription: String? {
        switch self {
...
extension GHKingfisherError.ProcessorErrorReason {
    var errorDescription: String? {
        switch self {
...
extension GHKingfisherError.ImageSettingErrorReason {
    var errorDescription: String? {
        switch self {
        case .emptySource:
