...
extension GHKingfisherWrapper where Base: CFCrossPlatformImageView {
    ...
    ///   - options: An options set to define image setting behaviors. See `GHKingfisherOptionsInfo` for more.
    ...
    ///
    /// - Note:
    /// This is the easiest way to use GHKingfisher to boost the image setting process from a source. Since all parameters
    ...
    /// ```
    ///
    /// Internally, this method will use `GHKingfisherManager` to get the source.
    ...
    @discardableResult
     func setImage(
        ...
        completionHandler: ((Result<RetrieveImageResult, GHKingfisherError>) -> Void)? = nil) -> DownloadTask?
    {
        var mutatingSelf = self
        guard let source = source else {
            mutatingSelf.placeholder = placeholder
            mutatingSelf.taskIdentifier = nil
            completionHandler?(.failure(GHKingfisherError.imageSettingError(reason: .emptySource)))
            return nil
        }

        var options = GHKingfisherParsedOptionsInfo(GHKingfisherManager.shared.defaultOptions + (options ?? .empty))
        ...
        let task = GHKingfisherManager.shared.retrieveImage(
            with: source,
            options: options,
            ...
            completionHandler: { result in
                CallbackQueue.mainCurrentOrAsync.execute {
                    ...
                    guard issuedIdentifier == self.taskIdentifier else {
                        let reason: GHKingfisherError.ImageSettingErrorReason
                        do {
                        ...
                        let error = GHKingfisherError.imageSettingError(reason: reason)
    ...
    ///
    /// - Note:
    /// This is the easiest way to use GHKingfisher to boost the image setting process from network. Since all parameters
    ...
    /// ```
    ///
    /// Internally, this method will use `GHKingfisherManager` to get the requested resource, from either cache
    ...
    /// - Returns: A task represents the image downloading.
    ///
    /// Internally, this method will use `GHKingfisherManager` to get the image data, from either cache
    ...
    private func needsTransition(options: GHKingfisherParsedOptionsInfo, cacheType: CacheType) -> Bool {
    ...
    /// It will be `nil` if `indicatorType` is `.none`.
     private(set) var indicator: Indicator? {
        ...
        set {
            ...
            // and casting protocol for associate object correctly. https://github.com/onevcat/GHKingfisher/issues/872
